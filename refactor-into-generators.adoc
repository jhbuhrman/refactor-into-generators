...
// FIXME: an excerpt from my ignite talk from some time ago:
...
* The value of the `yield` statement is returned back to the consumer
...
[source,python]
--
include::{sourcedir}/src/fibonacci/refactored.py[lines="20..25"]
--
// {nbsp} +
[.text-left]
--
...
//{empty}--
//``[``{startsb}{nbsp}__``start``__{nbsp}{endsb}``:``{nbsp}__``stop``__{nbsp}{startsb}``:``{nbsp}__``step``__{nbsp}{endsb}``]``
//{empty}--
but _without_ support for negative values for start, stop, or step.

//``more_itertools.``**`one`**``(``__``iterable``__``, ``__``too_short``__``=ValueError, ``__``too_long``__``=ValueError)``:
``more_itertools.``**`one`**``(``__``iterable``__{startsb}``, .{zwsp}..``{endsb}``)``:
...
[.notes]
--
* ``itertools``' **`islice`** selects elements from the iterable, like slicing
  would do from a sequence.
* And ``more_itertools``' **`one`** ensures that there is exactly one element
  in the iterable, and returns that element. It accepts additional
  keyword arguments to specify your own exceptions other than the
  default `ValueError`
* By the way, `more_itertools` provides an **`islice_extended`**,
  supporting negative values for start, stop, and step.
--
...
[source,python]
--
include::{sourcedir}/src/fibonacci/refactored.py[lines="28..33"]
--

[.notes]
--
* In this case, you use ``itertools``' **`islice`** "filter" with only
  the _stop_ parameter.
* And you use the `list` constructor to collect the elements in a list.
--


== The Largest Fib Less Than N (Python, Refactored)

[source,python]
--
include::{sourcedir}/src/fibonacci/refactored.py[lines="36..43"]
--
// {nbsp} +
[.text-left]
--
...
{empty}:: Make an iterator that returns elements from the iterable as
  long as the predicate is true.

//``more_itertools.``**`one`**``(``__``iterable``__``, ``__``too_short``__``=ValueError, ``__``too_long``__``=ValueError)``:
``more_itertools.``**`last`**``(``__``iterable``__{startsb}``, ``__``default``__{endsb}``)``:

{empty}:: Return the last item of iterable, or default if iterable is empty.
...
[source,python]
--
include::{sourcedir}/src/fibonacci/refactored.py[lines="46..50"]
--
// {nbsp} +
[.text-left]
--
...
{empty}:: Return the first item of iterable, or default if iterable is empty.
