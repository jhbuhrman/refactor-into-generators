...
[.notes]
--
* ``itertools``' **`islice`** selects elements from the iterable, like
  slicing would do from a sequence. Either you specify only the stop
  value, or you can specify start, stop, and optionally the step value
* And ``more_itertools``' **`one`** ensures that there is exactly one
  element in the iterable, and returns that element. It accepts
  additional keyword arguments to specify your own exceptions other than
...
[.notes]
--
* In this case, you use the ``itertools``' **`islice`** "filter
  function" with only the _stop_ parameter.
